...
export const FileCard = ({
  ...
  const icon = getFileIcon(
    normalizedFile.type || normalizedFile.contentType || ""
  );
  const size = formatFileSize(normalizedFile.size || 0);
    ...
    <div className="file-card">
      <div className="file-thumbnail">
        {normalizedFile.thumbnail || normalizedFile.blobUrl ? (
          <img
            src={normalizedFile.thumbnail || normalizedFile.blobUrl}
        ...
        <div className="file-actions">
          ...
          {normalizedFile.isShared ? (
            <Button
              variant="success"
              size="sm"
              onClick={() => onShare(normalizedFile)}
            >
              Shared
            </Button>
          ) : (
          ...
          <Button
            variant="primary"
            size="sm"
            onClick={() => onDownload(normalizedFile)}
          >
            Download
          </Button>
        </div>
